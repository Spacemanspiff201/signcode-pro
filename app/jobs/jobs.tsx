...
function Badge({ status }: { status: string }) {
  ...
  return (
    <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '20px', fontWeight: '500', whiteSpace: 'nowrap', background: colors.bg, color: colors.text }}>
      {status}
    </span>
  );
}
...
export default function JobsPage() {
  ...
  function toggleDoc(jobId: string, idx: number) {
    save(jobs.map(j => j.id === jobId ? {
      ...j,
      docs: j.docs.map((d, i) => i === idx ? { ...d, done: !d.done } : d),
  ...
  function addDoc(jobId: string, label: string) {
    ...
    save(jobs.map(j => j.id === jobId ? {
      ...j,
      docs: [...j.docs, { label: label.trim(), done: false }],
      updatedAt: new Date().toISOString(),
    } : j));
  }
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '60px 24px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '15px', fontWeight: '600', color: '#0D1B2A', marginBottom: '8px' }}>No jobs yet</div>
            <div style={{ fontSize: '13px', color: '#9BA8B4', marginBottom: '20px' }}>Create your first permit job to get started</div>
            <button onClick={openNew} style={{ padding: '9px 20px', background: '#185FA5', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>+ New job</button>
          </div>
        ) : (
                  ...
                  <div style={{ padding: '16px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0, marginLeft: '12px' }}>
                        <select value={job.status} onChange={e => updateStatus(job.id, e.target.value)} style={{ fontSize: '11px', border: 'none', background: 'transparent', color: STATUS_COLORS[job.status]?.text || '#444', fontWeight: '500', cursor: 'pointer' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '8px', marginBottom: '12px' }}>
                      {[
                        ...
                        { label: 'Target date', value: job.targetDate || '—' },
                    ...
                    <div style={{ borderTop: '1px solid #E2E8F0', padding: '16px 20px', background: '#F4F7FA' }}>
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          {job.docs.map((doc, idx) => (
                            <label key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '12px', color: doc.done ? '#3B6D11' : '#0D1B2A' }}>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="Add notes, calls, contacts, status updates..."
                          rows={3}
                          style={{ width: '100%', padding: '8px 10px', border: '0.5px solid #D3D1C7', borderRadius: '6px', fontSize: '12px', resize: 'vertical', background: '#fff', color: '#0D1B2A', outline: 'none', fontFamily: 'inherit' }}
                        />
