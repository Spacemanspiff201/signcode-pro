    ...
    <main style={{ fontFamily: 'Arial, Helvetica, sans-serif', background: '#F4F7FA', minHeight: '100vh' }}>
      ...
      <section style={{ padding: '72px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '15px', color: '#5A6B7A', lineHeight: '1.7', marginBottom: '48px' }}>
            Higher-paid employees spend hours digging through municipal websites. Newer staff struggle in unfamiliar jurisdictions. Rejected submittals add weeks to jobs. The information exists — it's just buried.
          </p>
      ...
      <section style={{ padding: '32px 24px', background: '#fff', borderTop: '1px solid #E2E8F0', borderBottom: '1px solid #E2E8F0' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', textAlign: 'center' }}>
          <p style={{ fontSize: '12px', color: '#9BA8B4', lineHeight: '1.7' }}>
            SignCode Pro provides general permit guidance based on publicly available sources to help sign professionals work more efficiently. Requirements vary by jurisdiction and change over time. Always verify current requirements directly with the jurisdiction before submitting. SignCode Pro is not a legal authority and does not guarantee permit approval.
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: '72px 24px', background: '#F4F7FA' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
            {[
              { name: 'Starter', price: '$79', period: '/mo', desc: 'Perfect for small shops and solo permit runners', features: ['Jurisdiction lookup', 'Document checklists', 'Up to 10 active jobs', 'Email support'], cta: 'Join waitlist', highlight: false },
              { name: 'Professional', price: '$199', period: '/mo', desc: 'For growing teams handling multiple jobs at once', features: ['Everything in Starter', 'Unlimited active jobs', 'Team dashboard', 'Job status tracking', 'Red flag alerts', 'Priority support'], cta: 'Join waitlist', highlight: true },
              { name: 'Enterprise', price: 'Custom', period: '', desc: 'For large operations and multi-location shops', features: ['Everything in Professional', 'Custom jurisdictions', 'API access', 'Dedicated support', 'Team training'], cta: 'Contact us', highlight: false },
              ...
              <div key={name} style={{ padding: '28px', background: highlight ? '#0D1B2A' : '#fff', borderRadius: '12px', border: highlight ? 'none' : '1px solid #E2E8F0', position: 'relative' }}>
                ...
                <a href="/waitlist" style={{ display: 'block', textAlign: 'center', padding: '10px', background: highlight ? '#185FA5' : '#F4F7FA', color: highlight ? '#fff' : '#0D1B2A', borderRadius: '8px', fontSize: '13px', fontWeight: '600', textDecoration: 'none' }}>{cta}</a>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 24px', background: '#0D1B2A' }}>
        <div style={{ maxWidth: '560px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '15px', color: '#85B7EB', marginBottom: '36px', lineHeight: '1.7' }}>
            Join sign professionals already on the waitlist. Be first when we launch.
          </p>
          {!submitted ? (
            <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '10px', maxWidth: '440px', margin: '0 auto' }}>
              <input
                ...
                required
                style={{ flex: 1, padding: '12px 16px', borderRadius: '8px', border: 'none', fontSize: '14px', outline: 'none' }}
              />
              <button type="submit" disabled={loading}
                style={{ padding: '12px 22px', background: '#185FA5', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap' }}>
