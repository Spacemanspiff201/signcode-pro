    ...
    <main style={{ fontFamily: "'DM Sans', 'Segoe UI', Arial, sans-serif", background: '#FAFBFD', color: '#0D1B2A', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .secondary-btn:hover { background:#fff !important; border-color:#B5D4F4 !important; }
        .jur-pill { transition: all .15s; cursor: default; }
      ...
      <nav style={{ background: 'rgba(250,251,253,0.94)', borderBottom: '1px solid rgba(0,0,0,0.07)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(14px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 48px', maxWidth: '1200px', margin: '0 auto', height: '62px' }}>
          ...
          <div className="nav-links" style={{ display: 'flex', gap: '32px' }}>
            {['Lookup', 'Jobs', 'Pricing', 'Waitlist'].map(l => (
              <a key={l} className="nav-link" href={`/${l.toLowerCase()}`} style={{ fontSize: '13px', color: '#5A6B7A', textDecoration: 'none', fontWeight: '500', transition: 'color .15s' }}>{l}</a>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <a href="/lookup" className="secondary-btn" style={{ padding: '7px 18px', border: '1px solid #D8E2EC', borderRadius: '8px', fontSize: '13px', color: '#0D1B2A', textDecoration: 'none', background: '#fff', fontWeight: '500' }}>Try lookup</a>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '700px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity .65s ease 220ms, transform .65s ease 220ms' }}>
            <p style={{ fontSize: '16px', color: '#5A6B7A', lineHeight: '1.75', marginBottom: '32px', maxWidth: '500px', margin: '0 auto 32px' }}>
              SignCode Pro identifies requirements, flags missing documents, and surfaces jurisdiction-specific gotchas — so your team submits cleaner, faster.
            </p>
          </div>
          <div className="hero-btns" style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity .65s ease 320ms, transform .65s ease 320ms', display: 'flex', gap: '12px', justifyContent: 'center', marginBottom: '16px', flexWrap: 'wrap' }}>
            <a href="/waitlist" className="primary-btn" style={{ padding: '13px 30px', background: '#185FA5', color: '#fff', borderRadius: '10px', fontSize: '14px', fontWeight: '600', textDecoration: 'none', display: 'inline-block', boxShadow: '0 4px 16px rgba(24,95,165,.32)' }}>Join the waitlist</a>
          ...
          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(12px)', transition: 'opacity .65s ease 480ms, transform .65s ease 480ms' }}>
            <form onSubmit={handleSearch}>
              <div className="search-row" style={{ display: 'flex', alignItems: 'center', background: '#fff', border: '1.5px solid #D8E2EC', borderRadius: '12px', padding: '10px 14px', maxWidth: '580px', margin: '0 auto', gap: '10px', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                ...
                <button type="submit" style={{ padding: '8px 20px', background: '#185FA5', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: '600', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>Look up</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* PAIN SECTION */}
      <section style={{ padding: '80px 48px', maxWidth: '1120px', margin: '0 auto' }}>
        <div className="pain-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'center' }}>
          <FadeIn>
            <div>
              ...
              <p style={{ fontSize: '14px', color: '#5A6B7A', lineHeight: '1.8', marginBottom: '28px' }}>Every unfamiliar jurisdiction means 2–4 hours hunting through outdated city websites, calling the wrong department, and hoping you caught everything. Then the submittal comes back rejected.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {['Submittal rejected → job delayed 2–4 weeks','Installer shows up, permit wasn\'t right, whole day lost','Master Sign Programs you didn\'t know existed','Engineer seals required — found out too late'].map(pain => (
          ...
          <FadeIn delay={120}>
            <div>
              ...
              <p style={{ fontSize: '14px', color: '#5A6B7A', lineHeight: '1.8', marginBottom: '28px' }}>SignCode Pro gives your team a clean starting point for every job — requirements, checklist, red flags, and direct contact info — in seconds instead of hours.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {['Instant requirements for any Florida jurisdiction','Auto-generated checklist by sign type','Red flags surfaced before you submit','Right contact, right department, right portal'].map(fix => (
          ...
          {[
            ...
            <FadeIn key={f.title} delay={i * 60}>
              <div className="feat-card" style={{ padding: '24px', border: '1.5px solid #E8EDF2', borderRadius: '14px', background: '#fff', height: '100%', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ fontSize: '14px', fontWeight: '700', color: '#0D1B2A', marginBottom: '7px' }}>{f.title}</div>
      ...
      {/* HOW IT WORKS */}
      <section style={{ padding: '80px 48px', background: '#F4F7FA', borderTop: '1px solid #E8EDF2', borderBottom: '1px solid #E8EDF2' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              { n: '01', title: 'Enter the job address and sign details', desc: 'Tell us where the sign is going and what type it is. SignCode Pro identifies the jurisdiction automatically.' },
              { n: '02', title: 'Get likely requirements instantly', desc: 'SignCode Pro surfaces permit requirements, common red flags, and required documents for that sign type and jurisdiction.' },
              { n: '03', title: "See what's missing before you submit", desc: 'A checklist shows exactly what you need to gather. Check items off as you collect them. Reduce rejected submittals.' },
              ...
              <FadeIn key={s.n} delay={i * 80}>
                <div className="step-row" style={{ display: 'flex', gap: '18px', background: 'rgba(255,255,255,.7)', border: '1.5px solid #E8EDF2', borderRadius: '14px', padding: '20px 24px', alignItems: 'flex-start', backdropFilter: 'blur(4px)' }}>
                  <div style={{ fontSize: '11px', fontWeight: '700', color: '#fff', minWidth: '34px', height: '26px', background: '#185FA5', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '2px', boxShadow: '0 2px 8px rgba(24,95,165,.3)' }}>{s.n}</div>
                  <div>
                    <div style={{ fontSize: '15px', fontWeight: '700', color: '#0D1B2A', marginBottom: '5px' }}>{s.title}</div>
          ...
          {[
            ...
            <FadeIn key={p.name} delay={i * 80}>
              <div className="price-card" style={{ position: 'relative', padding: '28px', background: p.highlight ? 'linear-gradient(160deg,#EBF3FB,#F5F9FE)' : '#fff', borderRadius: '16px', border: p.highlight ? '2px solid #185FA5' : '1.5px solid #E8EDF2', boxShadow: p.highlight ? '0 8px 32px rgba(24,95,165,.15)' : '0 2px 8px rgba(0,0,0,.04)', height: '100%' }}>
                {p.highlight && <div style={{ position: 'absolute', top: '-13px', left: '50%', transform: 'translateX(-50%)', background: '#185FA5', color: '#fff', fontSize: '11px', fontWeight: '700', padding: '4px 18px', borderRadius: '20px', whiteSpace: 'nowrap' }}>Most popular</div>}
            ...
            {!submitted ? (
              <form onSubmit={handleSubmit}>
                <div style={{ display: 'flex', gap: '10px', maxWidth: '420px', margin: '0 auto' }}>
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Your work email" required style={{ flex: 1, padding: '13px 16px', borderRadius: '9px', border: '1px solid rgba(255,255,255,.2)', fontSize: '14px', outline: 'none', background: 'rgba(255,255,255,.12)', color: '#fff' }}/>
                  <button type="submit" disabled={loading} style={{ padding: '13px 22px', background: '#fff', color: '#185FA5', border: 'none', borderRadius: '9px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>
