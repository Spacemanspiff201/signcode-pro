...
export default function Home() {
  ...
  return (
    <main style={{minHeight:'100vh',fontFamily:'Arial,sans-serif',display:'grid',gridTemplateColumns:'1fr 1fr'}}>
      <div style={{background:'#0D1B2A',padding:'48px 48px',display:'flex',flexDirection:'column',justifyContent:'center'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'10px',padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:'8px'}}>
          <div style={{flex:1}}>
            ...
            <div style={{height:'3px',background:'rgba(255,255,255,0.1)',borderRadius:'2px',marginTop:'8px',overflow:'hidden'}}>
              <div style={{width:'66%',height:'100%',background:'#85B7EB',borderRadius:'2px'}}></div>
            </div>
          </div>
        ...
        <div style={{marginTop:'28px',paddingTop:'20px',borderTop:'1px solid rgba(255,255,255,0.08)'}}>
          <div style={{fontSize:'10px',color:'rgba(255,255,255,0.25)',letterSpacing:'.06em',textTransform:'uppercase',marginBottom:'14px'}}>Trusted by sign companies working with</div>
          <div style={{display:'flex',gap:'20px',flexWrap:'wrap',alignItems:'center'}}>
            {['PUBLIX','AutoNation','WALGREENS','Chick-fil-A','Shell','TD Bank'].map(name => (
              <div key={name} style={{fontSize:'12px',fontWeight:'700',color:'rgba(255,255,255,0.2)',letterSpacing:'-.2px'}}>{name}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{background:'#fff',borderRadius:'16px',border:'1px solid #E2E8F0',padding:'36px',width:'100%',maxWidth:'420px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'12px'}}>
            {[['firstName','First name','Marcus'],['lastName','Last name','Davis']].map(([k,l,p]) => (
              <div key={k}>
                ...
                <input value={form[k as keyof typeof form]} onChange={e => update(k, e.target.value)} placeholder={p} style={{width:'100%',padding:'9px 12px',border:'1px solid #E2E8F0',borderRadius:'7px',fontSize:'13px',color:'#0D1B2A',background:'#fff',boxSizing:'border-box' as const}}/>
              </div>
            ))}
          </div>
          ...
          {[
            ...
            <div key={k} style={{marginBottom:'12px'}}>
              <label style={{fontSize:'12px',fontWeight:'500',color:'#5A6B7A',display:'block',marginBottom:'4px'}}>{l} *</label>
              <input type={t} value={form[k as keyof typeof form]} onChange={e => update(k, e.target.value)} placeholder={p} style={{width:'100%',padding:'9px 12px',border:'1px solid #E2E8F0',borderRadius:'7px',fontSize:'13px',color:'#0D1B2A',background:'#fff',boxSizing:'border-box' as const}}/>
            </div>
          ))}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'12px'}}>
            <div>
              ...
              <select value={form.role} onChange={e => update('role', e.target.value)} style={{width:'100%',padding:'9px 12px',border:'1px solid #E2E8F0',borderRadius:'7px',fontSize:'13px',color:'#0D1B2A',background:'#fff',boxSizing:'border-box' as const}}>
            ...
            <div>
              ...
              <select value={form.volume} onChange={e => update('volume', e.target.value)} style={{width:'100%',padding:'9px 12px',border:'1px solid #E2E8F0',borderRadius:'7px',fontSize:'13px',color:'#0D1B2A',background:'#fff',boxSizing:'border-box' as const}}>
                <option value="">Select...</option>
                <option>1–5</option>
                <option>6–15</option>
                <option>16–40</option>
          ...
          <div style={{marginBottom:'16px'}}>
            ...
            <input value={form.markets} onChange={e => update('markets', e.target.value)} placeholder="e.g. Miami-Dade, Broward, Palm Beach" style={{width:'100%',padding:'9px 12px',border:'1px solid #E2E8F0',borderRadius:'7px',fontSize:'13px',color:'#0D1B2A',background:'#fff',boxSizing:'border-box' as const}}/>
