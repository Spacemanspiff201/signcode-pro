...
export default function Home() {
  ...
  function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    if (searchQuery.trim()) {
      window.location.href = `/lookup?city=${encodeURIComponent(searchQuery.trim())}`;
    } else {
    ...
    <main style={{ fontFamily: "'DM Sans', 'Segoe UI', Arial, sans-serif", background: '#FAFBFD', color: '#0D1B2A', overflowX: 'hidden' }}>
          ...
          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(12px)', transition:'opacity .65s ease 480ms, transform .65s ease 480ms' }}>
            <form onSubmit={handleSearch}>
              <div className="search-row" style={{ display:'flex', alignItems:'center', background:'#fff', border:'1.5px solid #D8E2EC', borderRadius:'12px', padding:'10px 14px', maxWidth:'580px', margin:'0 auto', gap:'10px', boxShadow:'0 4px 20px rgba(0,0,0,0.06)' }}>
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="City or county — e.g. Miami, Fort Lauderdale, Pompano Beach..." style={{ flex:1, border:'none', background:'transparent', fontSize:'13.5px', color:'#0D1B2A', outline:'none', minWidth:0 }}/>
                <button type="submit" style={{ padding:'8px 20px', background:'#185FA5', color:'#fff', borderRadius:'8px', fontSize:'13px', fontWeight:'600', textDecoration:'none', whiteSpace:'nowrap', flexShrink:0, border:'none', cursor:'pointer' }}>Look up</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <section className="section" style={{ padding:'80px 48px', maxWidth:'1120px', margin:'0 auto' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'64px', alignItems:'center' }} className="pain-grid">
          <FadeIn>
            <div>
              <div style={{ fontSize:'11px', fontWeight:'700', color:'#185FA5', textTransform:'uppercase', letterSpacing:'.1em', marginBottom:'14px' }}>The problem</div>
              <h2 style={{ fontSize:'32px', fontWeight:'800', color:'#0D1B2A', letterSpacing:'-.5px', lineHeight:'1.2', marginBottom:'20px' }}>Permit research kills time your team doesn't have</h2>
              <p style={{ fontSize:'14px', color:'#5A6B7A', lineHeight:'1.8', marginBottom:'28px' }}>
                Every unfamiliar jurisdiction means 2–4 hours hunting through outdated city websites, calling the wrong department, and hoping you caught everything. Then the submittal comes back rejected.
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                {[
                  'Submittal rejected → job delayed 2–4 weeks',
                  'Installer shows up, permit wasn\'t right, whole day lost',
                  'Master Sign Programs you didn\'t know existed',
                  'Engineer seals required — found out too late',
                ].map(pain => (
                  <div key={pain} style={{ display:'flex', gap:'10px', alignItems:'flex-start' }}>
                    <span style={{ color:'#EF4444', fontWeight:'700', fontSize:'14px', flexShrink:0, marginTop:'1px' }}>✕</span>
                    <span style={{ fontSize:'13px', color:'#5A6B7A', lineHeight:'1.6' }}>{pain}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <FadeIn delay={120}>
            <div>
              <div style={{ fontSize:'11px', fontWeight:'700', color:'#22C55E', textTransform:'uppercase', letterSpacing:'.1em', marginBottom:'14px' }}>The fix</div>
              <h2 style={{ fontSize:'32px', fontWeight:'800', color:'#0D1B2A', letterSpacing:'-.5px', lineHeight:'1.2', marginBottom:'20px' }}>Know what you need before you start</h2>
              <p style={{ fontSize:'14px', color:'#5A6B7A', lineHeight:'1.8', marginBottom:'28px' }}>
                SignCode Pro gives your team a clean starting point for every job — requirements, checklist, red flags, and direct contact info — in seconds instead of hours.
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                {[
                  'Instant requirements for any Florida jurisdiction',
                  'Auto-generated checklist by sign type',
                  'Red flags surfaced before you submit',
                  'Right contact, right department, right portal',
                ].map(fix => (
                  <div key={fix} style={{ display:'flex', gap:'10px', alignItems:'flex-start' }}>
                    <span style={{ color:'#22C55E', fontWeight:'700', fontSize:'14px', flexShrink:0, marginTop:'1px' }}>✓</span>
                    <span style={{ fontSize:'13px', color:'#0D1B2A', lineHeight:'1.6', fontWeight:'500' }}>{fix}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* TESTIMONIALS */}
      <section className="section" style={{ padding:'80px 48px', background:'#F4F7FA', borderTop:'1px solid #E8EDF2' }}>
        <div style={{ maxWidth:'1120px', margin:'0 auto' }}>
          <FadeIn>
            <div style={{ textAlign:'center', marginBottom:'48px' }}>
              <div style={{ fontSize:'11px', fontWeight:'700', color:'#185FA5', textTransform:'uppercase', letterSpacing:'.1em', marginBottom:'10px' }}>From the field</div>
              <h2 style={{ fontSize:'34px', fontWeight:'800', color:'#0D1B2A', letterSpacing:'-.5px' }}>Built by someone who's lived it</h2>
            </div>
          </FadeIn>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px' }} className="feat-grid">
            {[
              { quote:"Showing up to a job and finding out the permit wasn't right is the worst feeling. You lose the whole day and the customer loses faith.", name:'Joe S.', role:'20-year commercial sign installer', initials:'JS', highlight:true },
              { quote:"Boca Raton alone can take 8–12 weeks and they'll reject you for anything. You need to know the rules before you even quote the job.", name:'Permit coordinator', role:'South Florida sign company', initials:'PC', highlight:false },
              { quote:"Every municipality has its own quirks. Fort Lauderdale wants contractor registration. Miami-Dade needs an engineer seal over 24 sq ft. You can't just guess.", name:'Sign company owner', role:'Broward County operation', initials:'SC', highlight:false },
            ].map((t, i) => (
              <FadeIn key={i} delay={i * 80}>
                <div className="feat-card" style={{ padding:'28px', border: t.highlight ? '2px solid #185FA5' : '1.5px solid #E8EDF2', borderRadius:'16px', background: t.highlight ? 'linear-gradient(160deg,#EBF3FB,#F5F9FE)' : '#fff', height:'100%', boxShadow: t.highlight ? '0 8px 32px rgba(24,95,165,.12)' : '0 2px 8px rgba(0,0,0,.04)', position:'relative' }}>
                  <div style={{ fontSize:'28px', color:'#185FA5', opacity:.25, fontFamily:'Georgia,serif', lineHeight:1, marginBottom:'12px' }}>"</div>
                  <p style={{ fontSize:'13.5px', color:'#0D1B2A', lineHeight:'1.75', fontStyle:'italic', marginBottom:'20px', fontWeight:'450' }}>{t.quote}</p>
                  <div style={{ display:'flex', alignItems:'center', gap:'10px', marginTop:'auto' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'50%', background: t.highlight ? '#185FA5' : '#E8EDF2', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:'700', color: t.highlight ? '#fff' : '#5A6B7A', flexShrink:0 }}>{t.initials}</div>
                    <div>
                      <div style={{ fontSize:'12.5px', fontWeight:'700', color:'#0D1B2A' }}>{t.name}</div>
                      <div style={{ fontSize:'11px', color:'#9BA8B4' }}>{t.role}</div>
                    </div>
                  </div>
                </div>
              </FadeIn>
      ...
      <section className="section" style={{ padding:'80px 48px', maxWidth:'1120px', margin:'0 auto' }}>
        <FadeIn>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:'20px', marginBottom:'36px' }}>
            <div>
              <div style={{ fontSize:'11px', fontWeight:'700', color:'#185FA5', textTransform:'uppercase', letterSpacing:'.1em', marginBottom:'10px' }}>Coverage</div>
              <h2 style={{ fontSize:'34px', fontWeight:'800', color:'#0D1B2A', letterSpacing:'-.5px', lineHeight:'1.2', maxWidth:'420px' }}>10 Florida jurisdictions — more being added</h2>
            </div>
            <a href="/lookup" style={{ fontSize:'13px', color:'#185FA5', fontWeight:'600', textDecoration:'none', whiteSpace:'nowrap', display:'flex', alignItems:'center', gap:'5px' }}>View all <span>→</span></a>
          </div>
        </FadeIn>
        <FadeIn delay={80}>
          <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginBottom:'28px' }}>
            {[
              { label:'Miami-Dade County', badge:'Complex' },
              { label:'Broward County', badge:'' },
              { label:'Palm Beach County', badge:'' },
              { label:'City of Fort Lauderdale', badge:'' },
              { label:'City of Pompano Beach', badge:'MSP' },
              { label:'City of Boca Raton', badge:'Strict' },
              { label:'City of Miami Beach', badge:'Historic' },
              { label:'City of Orlando', badge:'' },
              { label:'City of Tampa', badge:'' },
              { label:'Hillsborough County', badge:'⚠️' },
            ].map(j => (
              <div key={j.label} className="jur-pill" style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', border:'1.5px solid #E8EDF2', borderRadius:'24px', background:'#fff', fontSize:'12.5px', color:'#0D1B2A', fontWeight:'500', cursor:'default', boxShadow:'0 1px 4px rgba(0,0,0,.03)' }}>
                <span style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#22C55E', flexShrink:0 }}/>
                {j.label}
                {j.badge && <span style={{ fontSize:'10px', fontWeight:'700', color:'#185FA5', background:'#EBF3FB', padding:'1px 6px', borderRadius:'10px' }}>{j.badge}</span>}
              </div>
            ))}
            <div className="jur-pill" style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', border:'1.5px dashed #D8E2EC', borderRadius:'24px', background:'#FAFBFD', fontSize:'12.5px', color:'#9BA8B4', fontWeight:'500', cursor:'default' }}>
              <span style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#D8E2EC', flexShrink:0 }}/>
              + More coming soon
            </div>
          </div>
        </FadeIn>
        <FadeIn delay={120}>
          <div style={{ background:'linear-gradient(135deg,#EBF3FB,#F5F9FE)', border:'1.5px solid #C5DDF5', borderRadius:'14px', padding:'20px 24px', display:'flex', alignItems:'center', gap:'16px', flexWrap:'wrap' }}>
            <div style={{ fontSize:'20px' }}>📍</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:'13px', fontWeight:'700', color:'#0D1B2A', marginBottom:'3px' }}>Don't see your jurisdiction?</div>
              <div style={{ fontSize:'12.5px', color:'#5A6B7A' }}>Join the waitlist and tell us what markets you work in — we prioritize coverage based on demand.</div>
            </div>
            <a href="/waitlist" className="primary-btn" style={{ padding:'9px 20px', background:'#185FA5', color:'#fff', borderRadius:'9px', fontSize:'13px', fontWeight:'600', textDecoration:'none', whiteSpace:'nowrap', boxShadow:'0 2px 8px rgba(24,95,165,.28)', flexShrink:0 }}>Request a jurisdiction</a>
          </div>
        </FadeIn>
      </section>
